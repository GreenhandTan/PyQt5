...
class BorrowInfoWindow(Ui_Form, QWidget):
    ...
    def return_book(self, borrow_id):
        print("记录id：{}".format(borrow_id))
        db = DBHelp()
        count, res = db.query_super("borrow_info", "id", borrow_id)
        if count > 0:
            print(res)
            print("图书id：" + res[0][1])
            borrow_id = res[0][1]
        db.update_borrow_statue(borrow_id)
        db.update_borrow_return(borrow_id)
        data = [res[0][3], borrow_id, 1, get_current_time()]
        db.insert_all_return_info(data)
        db.delete("borrow_info", "book_id", borrow_id)
        db.db_commit()
        db.instance = None
        del db
        self.init_data()
